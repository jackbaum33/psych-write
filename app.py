...
def batched_generate_paragraphs(section_prompts, appendix):
    numbered_sections = [
        f"### {i+1}. {label}\n{content}" for i, (content, label) in enumerate(section_prompts)
    ]
    prompt = (
        ...
        "Respond ONLY with each paragraph in the same order as labeled below.\n\n"
        f"{''.join(numbered_sections)}\n\n"
    ...
    generated = [p.strip() for p in response.choices[0].message.content.strip().split("\n") if p.strip()]
    return {
        placeholder: paragraph
        for (_, placeholder), paragraph in zip(section_prompts, generated)
    }
...
def generate():
    ...
    # Prepare prompts for narrative paragraphs
    section_prompts = []
    ...
    # Generate paragraphs and map them to placeholders
    placeholder_paragraphs = batched_generate_paragraphs(section_prompts, appendix)

    for paragraph_obj in doc.paragraphs:
        for placeholder, paragraph in placeholder_paragraphs.items():
