...
def generate():
    ...
    for (_, placeholder), paragraph_text in zip(section_prompts, paragraphs):
        for paragraph in doc.paragraphs:
            for run in paragraph.runs:
                if placeholder in run.text:
                    run.text = run.text.replace(placeholder, paragraph_text)
